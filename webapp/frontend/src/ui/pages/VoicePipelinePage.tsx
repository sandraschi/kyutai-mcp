...
function MoshiServiceBadge() {
  ...
  if (!status)
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1 text-sm text-slate-300">
    ...
    <div className="grid h-full gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <div className="flex min-w-0 flex-col gap-6 overflow-auto pr-2 pb-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-sm font-medium text-slate-300 bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
              Provider routing:
              <select
        ...
        <section className="rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/5 to-fuchsia-500/5 p-6 backdrop-blur">
          <div className="flex items-center justify-between mb-4">
            ...
            {proxyStatus?.is_running && (
              <span className="flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-3 py-1 text-sm font-medium text-emerald-300 ring-1 ring-emerald-500/30">
          ...
          <div className="space-y-4">
            ...
            {transcriptData && proxyStatus?.is_running && (
              <div className="mt-4 rounded-xl border border-white/10 bg-slate-950/50 p-4 max-h-64 overflow-y-auto">
                <h4 className="text-sm font-semibold uppercase text-slate-400 mb-3 border-b border-white/5 pb-2">
                ...
                {transcriptData.transcript?.length > 0 ? (
                    ...
                    {transcriptData.transcript.map((item: any, idx: number) => (
                      <div key={idx} className="text-sm">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] text-slate-400">{new Date(item.timestamp * 1000).toLocaleTimeString()}</span>
                        </div>
                  ...
                  <p className="text-sm text-slate-400 italic">No conversation captured yet.</p>
                )}
              </div>
            )}
          </div>
              ...
              {turnResult && (
                <div className="rounded-xl border border-violet-500/10 bg-violet-500/5 p-4 animate-fade-in-up">
                  ...
                  <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-violet-500/20 px-2.5 py-0.5 text-sm font-medium text-violet-300">
                ...
                <div className="rounded-xl border border-amber-500/10 bg-amber-500/5 p-4 animate-fade-in-up">
                  ...
                  {bpResult.sources && bpResult.sources.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-amber-500/10 flex flex-wrap gap-2">
                      <span className="text-[10px] uppercase text-slate-400 w-full mb-1">Live data sources used:</span>
                      {bpResult.sources.map((s: any, i: number) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1 rounded border border-white/5 bg-black/40 px-2 py-1 text-[10px] text-slate-300 hover:text-amber-200 transition"
                        >
                          {s.name}
                        </a>
                      ))}
      ...
      <aside className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-slate-900/30 p-5 backdrop-blur">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            className="rounded p-1 text-slate-400 hover:bg-white/10 hover:text-slate-300 transition"
          ...
          {sessions.map((s) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-black/50 px-2 py-0.5 text-[10px] font-medium text-slate-300">
                  {s.turn_count} turns
                </span>
              </div>
              {s.last_activity_ms && (
        ...
        {selectedSession && sessionTurns.length > 0 && (
          <div className="flex-shrink-0 max-h-[40%] flex flex-col pt-3 border-t border-white/10 animate-fade-in">
            <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3 truncate">
              {selectedSession}
            </h4>
            <div className="space-y-3 overflow-auto pr-1 pb-1">
              {sessionTurns.map((t, i) => (
                <div
                  key={i}
                  className="rounded-xl border border-white/5 bg-black/30 p-3 text-sm"
                >
