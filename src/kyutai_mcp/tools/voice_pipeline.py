...
async def _fetch_stocks(symbols: list[str]) -> list[dict[str, Any]]:
    ...
    If no key is configured, returns a stub explaining the situation rather
    than hitting an unofficial endpoint that may be blocked.
    ...
    normalized = [s.strip().upper() for s in symbols if s.strip()] or ["^GSPC", "AAPL", "NVDA"]
    results: list[dict[str, Any]] = []
    async with httpx.AsyncClient() as client:
        for symbol in normalized[:5]:  # cap at 5 to stay within free rate limits
            try:
                r = await client.get(
